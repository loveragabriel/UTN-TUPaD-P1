...
productos_ingresado = stock_productos
print(f"El Stock de productos: {productos_ingresado}")
...
if producto_agregar in stock_productos:
    ...
    print(f"Este es el Stock actualizado: {productos_ingresado}")
...
agenda_clave_valor = {
    ...
    ("Miércoles", "16:00"): "Clase de inglés",
    ("Jueves", "13:00"): "Almuerzo con cliente",
    ("Viernes", "18:00"): "Cierre de semana y planificación"
}

#Función Consulta
def consulta_actividad(dia, hora):
    clave = (dia, hora)
    if clave in agenda_clave_valor:
        print(f"{dia} a las {hora}: {agenda_clave_valor[clave]}")
    else:
        print(f"No hay actividades para el día {dia} a las {hora}h.")

consulta_dia = input("Ingrese el día que desea consultar en la agenda:")
consulta_hora = input("Ingrese la hora que sea consultar:")
consulta_actividad(consulta_dia, consulta_hora)
...
capitales_paises_sudamerica = {}

for pais, capital in paises_capitales_sudamerica.items():
    capitales_paises_sudamerica[capital] = pais
print(f"Listado paises  y capitales {paises_capitales_sudamerica}")
print(f"Lista invertidad: {capitales_paises_sudamerica}")